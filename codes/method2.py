...
def copy_periods(RG, sp):
    ...
    for i in range(len(RG.matrix)):
        for j in range(len(RG.matrix[0]), l):
            #new_RG.matrix[i][j] = RG.matrix[i][j % len(RG.matrix[0])]
            if RG.matrix[i][j % len(RG.matrix[0])] != None:
                new_RG.matrix[i][j] = State(range, name = RG.matrix[i][j % len(RG.matrix[0])].name, block= RG.matrix[i][j % len(RG.matrix[0])].block)

                c = (j - j% len(RG.matrix[0]))/len(RG.matrix[0])
                new_RG.matrix[i][j].name = new_RG.matrix[i][j].name[:2] + str(int(new_RG.matrix[i][j].name[2:]) + c * len(RG.matrix[0]))
                if new_RG.matrix[i][j] != None:
                    new_RG.matrix[i][j].range = libc.copy_context(RG.matrix[i][j % len(RG.matrix[0])].range)


    for i in range(len(RG.matrix)):
        for j in range(0, len(RG.matrix[0])):
    ...
    for edge in RG.edges:
        for c in range(1, n):
            ...
            newedge.start.name = newedge.start.name[:2] + str(int(newedge.start.name[2:]) + c * len(RG.matrix[0]))
            ...
            newedge.end.name = newedge.end.name[:2] + str(int(newedge.end.name[2:]) + c * len(RG.matrix[0]))
...
class TestBlock(Block):
    ...
    def __init__(self, name = 'B1'):
        ...
        edge1 = Edge([], ['e1',4] , ['1_action2', 1.5],  'o1', '', s1, s2, None)
        edge2 = Edge([], ['e1',4] , ['1_action3', 3.5], 'o2', '1_condition3', s2, s3, None)
        edge3 = Edge([],['e1',4] , ['1_action4', 3.5], 'o1', '', s3, s2, None)
        edge4 = Edge([],['e1',4] , ['1_action1', 2], 'o2', '1_condition1', s2, s1, None)
...
class TestBlock1(Block):
    ...
    def __init__(self, name = 'B2'):
        ...
        edge1 = Edge([], ['e3',2] , ['', 0.8], 'o1', '', s1, s2)
        edge2 = Edge([],['e3',2] , ['2_action2', 1.2], 'o2', '2_condition2', s2, s3)
        edge3 = Edge([],['e3',2] , ['2_action1', 0.5], 'o1', '2_condition1', s3, s2)
        edge4 = Edge([],['e3',2] , ['2_action5', 1.8], 'o2', '2_condition5', s2, s4)
        edge5 = Edge([],['e3',2] , ['2_action3', 1.8], 'o2', '2_condition3', s3, s1)
        edge6 = Edge([],['e3',2] , ['', 1.3], 'o2', '', s4, s3)
        #edge5 = Edge(['e2',5] , ['a5', 0.15], 'o2', '', s1, s2)
        periods = [2]
...
class TestBlock2(Block):
    ...
    def __init__(self, name = 'B3'):
        ...
        edge1 = Edge([],['e4',3] , ['', 2.5], 'o1', '3_condition2', s1, s2)
        edge2 = Edge([],['e4',3] , ['3_action3', 2.5], 'o2', '', s2, s1)
...
def compute_rbf(start, end, RG, floyd):
    ...
    for i in range(len(RG.matrix)):
        for j in range(len(RG.matrix)):
            if RG.matrix[i][st] != None and RG.matrix[j][ed] != None:
...
for l in range(1, len(RG_LIST)):
    start_times = combine_instant(RG_LIST[0:l+ 1])
    match = {}
