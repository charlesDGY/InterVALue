    ...
    def trans_to_rea(self, P):
                    ...
                    #i = i + 1
                        ...
                        #RG.matrix[self.trans_to_num(source.name)][temp] = s
                            ...
                            for p_e in range(len(store)):
                                ...
                                if rl_e is None:
                                    r_e = r_e + 1
                                elif rl_e.name == edge.end.name + str(n) and rl_e.block == source. block and rl_e.range == source.range:
                                    e = rl_e
                                else:
                                    r_e = r_e + 1
                            if r_e == len(store):
                        ...
                        #RG.matrix[self.trans_to_num(edge.end.name)][time] = e
                            ...
                            if e.name in s.succs:
                                wcet = max(s.succs[e.name], edge.action[-1])
        ...
        for x in range(len(store)):
            for y in range(len(ins) - 1):
                if RG.matrix[x][y + 1].name not in RG.matrix[x][y].succs:
...
class TestBlock(Block):
    ...
    def __init__(self, name = 'B1'):
        ...
        edge1 = Edge(['e1',4] , ['a1', 1.5], 'o1', '', s1, s2)
        edge2 = Edge(['e1',4] , ['a2', 3.5], 'o2', '', s3, s2)
        edge3 = Edge(['e1',4] , ['a3', 3.5], 'o1', '', s2, s3)
        edge4 = Edge(['e1',4] , ['a4', 2], 'o2', '', s2, s1)
...
class TestBlock1(Block):
    ...
    def __init__(self, name = 'B1'):
        ...
        edge1 = Edge(['e1',2] , ['a1', 0.8], 'o1', '', s1, s2)
        edge2 = Edge(['e2',2] , ['a2', 1.2], 'o2', '', s2, s3)
        edge3 = Edge(['e1',2] , ['a3', 0.5], 'o1', '', s3, s2)
        edge4 = Edge(['e2',2] , ['a4', 1.8], 'o2', '', s2, s4)
        edge5 = Edge(['e2',2] , ['a4', 1.8], 'o2', '', s3, s1)
        edge6 = Edge(['e2',2] , ['a4', 1.3], 'o2', '', s4, s3)
        periods = [2]
...
class TestBlock2(Block):
    ...
    def __init__(self, name = 'B1'):
        ...
        s2 = State('s2', None, 'x > 1')
        s3 = State('s3', None, 'x > 1')
        edge1 = Edge(['e1',3] , ['a1', 2.5], 'o1', '', s1, s2)
        edge2 = Edge(['e2',3] , ['a2', 2.5], 'o2', '', s2, s1)
