    ...
    for i in range(len(RG.matrix)):
        for edge in get_state_edge_rea(RG.edges, RG.matrix[i][st]):
            if (edge.guard != ''):
                ...
                end_context = libc.new_range_analysis(edge.start.range, c_int(0), edge.source.cfg_list[0][0], edge.source.cfg_list[0][1])
            ...
            if bool(end_context.contents.next):
                ...
                fresh.append([0, start, edge.start.name, edge.start.range])
                result.append(edge.wcet)
                ...
                if edge.start.name == path[2]:
                    if path[1] + interval <= end:
                        ...
                        if bool(end_context.contents.next):
                            ...
                            print 'gogogo'
                            next_input_range.contents.next.contents.next = POINTER(valueanalysis.edge_context)()
                            fresh.append([path[0] + edge.wcet, path[1] + interval, edge.end.name, next_input_range])
